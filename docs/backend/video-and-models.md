...
signaling (8889) stay inside Docker. Without the overlay, existing deployments
...
for video/models and `SendCommands` for model uploads. Sign in on the media page;
its session is held in page memory independently of other tabs. Guest access
follows the existing backend configuration. Mission's iframe player and Vehicle's
...
with `MTX_APIADDRESS=127.0.0.1:9997` and `MTX_WEBRTCADDRESS=127.0.0.1:8889`. Set
`GS_VIDEO_PASSWORD` on the backend. Its default relay URLs are those loopback
addresses; override `GS_VIDEO_API_URL` and `GS_VIDEO_WEBRTC_URL` if necessary.
...
select a stored model, then reopen the Vehicle tab. Without an explicit selection,
...
`stream_labels`, and `stats` fields configure Mission labels and live-stat bindings.
GET `/api/live_streams` returns a valid empty stream list if the receiver is down,
allowing Mission to fall back to the vehicle view.
...
| `GET /api/vehicle_visualization` | Frontend Vehicle model/stage/binding contract |
...
The included client reconnects automatically. Video is live-only, not recorded.
