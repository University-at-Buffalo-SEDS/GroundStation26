...
This document describes how the frontend is structured, how data moves through it, and which source files own which responsibilities.
...
The frontend is intentionally thin on business logic. It is mainly a stateful UI/runtime around backend-provided telemetry, layout, and control policy.
...
The frontend avoids doing heavy work directly inside the websocket receive hot path. High-rate telemetry is buffered and then flushed into render-facing state at a controlled cadence.
...
The important design choice is that telemetry rows are not written directly into heavy UI structures on every websocket message. Instead:
...
This keeps the same map source model across browser and native builds while letting native apps bypass mixed-content/browser-origin restrictions.
