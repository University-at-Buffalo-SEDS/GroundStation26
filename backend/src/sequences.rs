...
pub fn command_name(cmd: &TelemetryCommand) -> &'static str {
    match cmd {
        ...
        TelemetryCommand::CO2 => "DeployParachute",
        TelemetryCommand::Reef => "ExpandParachute",
        TelemetryCommand::ReinitSensors => "ReinitSensors",
        TelemetryCommand::EvalRelax => "EvaluationRelax",
        TelemetryCommand::EvalFocus => "EvaluationFocus",
        TelemetryCommand::EvalAbort => "EvaluationAbort",
        TelemetryCommand::ReinitBaro => "ReinitBarometer",
        TelemetryCommand::EnableIMU => "EnableIMU",
        TelemetryCommand::DisableIMU => "DisableIMU",
        TelemetryCommand::VigilantMode => "MonitorAltitude",
        TelemetryCommand::SimpletonMode => "RevokeMonitorAltitude",
        TelemetryCommand::ConsecutiveSamp => "ConsecutiveSamples",
        TelemetryCommand::AccumulativeSamp => "RevokeConsecutiveSamples",
        TelemetryCommand::ResetFailures => "ResetFailures",
        TelemetryCommand::AccumulateFailures => "RevokeResetFailures",
        TelemetryCommand::ValidateMeasms => "ValidateMeasms",
        TelemetryCommand::SkipValidation => "RevokeValidateMeasms",
        TelemetryCommand::ThresAbort15 => "AbortAfter15",
        TelemetryCommand::ThresAbort40 => "AbortAfter40",
        TelemetryCommand::ThresAbort70 => "AbortAfter70",
        TelemetryCommand::ThresReinit12 => "ReinitAfter12",
        TelemetryCommand::ThresReinit26 => "ReinitAfter26",
        TelemetryCommand::ThresReinit44 => "ReinitAfter44",
    }
}
...
fn build_policy(
    ...
    if !is_fill_state(flight_state) {
        ...
        if flight_state == FlightState::Idle && valves.normally_open != Some(false) {
            enabled.insert(
                "NormallyOpen",
    ...
    match runtime.step {
        SequenceStep::SetupValves => {
            ...
            if valves.dump_open != Some(false) {
                recommended.insert("Dump", pending_mode(state, "Dump", now_ms, cfg));
            }
        }
        SequenceStep::NitrogenFill => {
            if valves.nitrogen_open != Some(true) {
                recommended.insert("Nitrogen", pending_mode(state, "Nitrogen", now_ms, cfg));
            }
        }
        SequenceStep::CloseNitrogen => {
            if valves.nitrogen_open != Some(false) {
        ...
        SequenceStep::RetractFillLines => {
            if valves.retract != Some(true) {
                recommended.insert(
                    "RetractPlumbing",
                    pending_mode(state, "RetractPlumbing", now_ms, cfg),
                );
            }
        }
        SequenceStep::ArmedReady => {
