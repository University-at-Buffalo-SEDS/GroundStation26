use super::{AlertMsg, format_timestamp_ms_clock, translate_text};
use dioxus::prelude::*;
...
pub fn WarningsTab(warnings: Signal<Vec<AlertMsg>>) -> Element {
    rsx! {
        div { style: "padding:16px;",
            h2 { style: "margin:0 0 12px 0;", "{translate_text(\"Warnings\")}" }
            ...
            div { style: "display:flex; flex-direction:column; gap:10px;",
                ...
                if warnings.read().is_empty() {
                    div { style: "color:#94a3b8;", "{translate_text(\"No warnings.\")}" }
                }
            }
        }
    }
