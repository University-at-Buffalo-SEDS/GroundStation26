...
static void gs26_poke_host_port(NSString *host, uint16_t port, uint32_t timeout_ms) {
    if (host.length == 0) return;
    if (port == 0) port = 80;
    if (timeout_ms == 0) timeout_ms = 900;
...
void gs26_localnet_poke_url(const char *url_cstr) {
    @autoreleasepool {
        ...
        if (host.length == 0) {
            NSURLComponents *c2 =
                [NSURLComponents componentsWithString:[@"http://" stringByAppendingString:urlStr]];
            host = c2.host;
            portNum = c2.port;
            c = c2;
        }
